...
def hedging_criteria(port_mat: np_ndarray) -> float:
    # ['d_p', 'delta', 'gamma', 'rho', 'theta', 'vega']
    return port_mat[1]**2


def optimize_one_day(i: int, op_ct_func, pw_mat: np_ndarray, o_cub: np_ndarray, hg_tools: Tuple = (-1,)) -> np_ndarray:
    print(i)
    port_mat = o_cub[:, i, :]
    pw_ar = pw_mat[:, i, :]
...
def calc_hedge_w_n_expo(op_iv_dct: Dict[str, pd_DataFrame], u_p_df: pd_DataFrame, pw_mat: np_ndarray,
                        hg_tools: Tuple = (-1,)) -> Tuple[pd_DataFrame, pd_DataFrame]:
    o_cub, cols = gen_risk_metrics(op_iv_dct, u_p_df)
    o_cub = o_cub.astype(float)
    o_cub[np_isnan(o_cub)] = 0.0

    pool = Pool()
    hw_mat = np_array(pool.map(partial(optimize_one_day, op_ct_func=hedging_criteria,
                                       hg_tools=hg_tools, pw_mat=pw_mat, o_cub=o_cub), list(range(o_cub.shape[1]))))
    pool.close()
    pool.join()
